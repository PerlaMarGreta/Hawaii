...
@st.cache_data
def cargar_datos_y_modelo(archivo_csv):
    # Cargar datos locales
    datos = pd.read_csv(archivo_csv)
    
    # Mapear precios a valores numéricos
    ...
    X = datos[features].dropna()
    y = datos.loc[X.index, target]
...
# Cargar datos locales
archivo_csv = "HawaiiMuestra.csv"  # Asegúrate de que el archivo esté en la misma carpeta
Hawaii, model = cargar_datos_y_modelo(archivo_csv)
...
for _, row in Hawaii.iterrows():
    features_input = pd.DataFrame([[row['rating'], row['num_of_reviews'], row['price_numeric'],
                                    row['latitude'], row['longitude']]], columns=['rating', 'num_of_reviews', 'price_numeric', 'latitude', 'longitude'])
